...
describe('React', () => {
  ...
  describe('<AdoptionAgency /> component', () => {
    ...
    it('binds the `adoptPet` method to the context of the component', () => {
      expect(adoptionAgency.instance().adoptPet.hasOwnProperty('prototype')).to
        .be.false;
    });
    ...
    it('renders the name of the petToAdopt in an h3 tag', () => {
      ...
      expect(adoptionAgency.find('h3').text().trim()).to.be.equal(
        'You are adopting: Cody'
      );
    });
    ...
    it('passes a prop called `pets` with the list of dogs to one of the <PetPreview /> components', () => {
      expect(
        adoptionAgency.find('#dogs').find(PetPreview).props().pets
      ).to.be.deep.equal(DOGS);
    });

    it('passes a prop called `pets` with the list of cats the other <PetPreview /> component', () => {
      expect(
        adoptionAgency.find('#cats').find(PetPreview).props().pets
      ).to.be.deep.equal(CATS);
    });

    it('passes the `adoptPet` method to each <PetPreview/> component as a prop called `adoptPet`', () => {
      expect(
        adoptionAgency.find('#dogs').find(PetPreview).props().adoptPet
      ).to.be.deep.equal(adoptionAgency.instance().adoptPet);

      expect(
        adoptionAgency.find('#cats').find(PetPreview).props().adoptPet
      ).to.be.deep.equal(adoptionAgency.instance().adoptPet);
    });
  ...
  describe('<PetPreview /> component', () => {
    ...
    it('binds the `previewPet` method to the context of the component', () => {
      expect(petPreview.instance().previewPet.hasOwnProperty('prototype')).to.be
        .false;
    });

    it('binds the `adoptSelectedPet` method to the context of the component', () => {
      expect(petPreview.instance().adoptSelectedPet.hasOwnProperty('prototype'))
        .to.be.false;
    });
    ...
    it('passes its own `pets` prop down to <AdoptionForm /> as `pets`', () => {
      expect(petPreview.find(AdoptionForm).props().pets).to.be.deep.equal(
        petPreview.instance().props.pets
      );
    });

    it('passes the `previewPet` method down to <AdoptionForm /> as a prop called `previewPet`', () => {
      expect(petPreview.find(AdoptionForm).props().previewPet).to.be.deep.equal(
        petPreview.instance().previewPet
      );
    });

    it('passes the `adoptSelectedPet` method down to <AdoptionForm /> as a prop called `adoptSelectedPet`', () => {
      expect(
        petPreview.find(AdoptionForm).props().adoptSelectedPet
      ).to.be.deep.equal(petPreview.instance().adoptSelectedPet);
    });
  ...
  describe('<AdoptionForm /> component', () => {
    ...
    beforeEach('Create component', () => {
      ...
      adoptionForm = shallow(
        <AdoptionForm
          pets={CATS}
          previewPet={previewPetSpy}
          adoptSelectedPet={adoptSelectedPetSpy}
        />
      );
    });
    ...
    it('renders an <option> within the <select> for each pet', () => {
      expect(adoptionForm.find('select').find('option').length).to.be.equal(
        CATS.length
      );
    });
