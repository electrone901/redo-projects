...
describe('React components', () => {
  describe('Article', () => {
    ...
    it("includes the article's title as an h1", () => {
      expect(articleWrapper.find('h1').text().trim()).to.be.equal(
        'Migratory Birds'
      );
    });
    ...
    it('is not hardcoded', () => {
      ...
      const differentarticleWrapper = shallow(
        <Article fullArticle={aDifferentArticle} />
      );
      expect(differentarticleWrapper.find('h1').text().trim()).to.be.equal(
        'Blue Wizards'
      );
  ...
  describe('ArticleList', () => {
    ...
    it('is comprised of <Article /> components based on what gets placed on the state', () => {
      ...
      console.log('after***', articleListWrapper, firstMessage);
      expect(firstMessage.equals(<Article fullArticle={articleList[0]} />)).to
        .be.true; // eslint-disable-line
    });
